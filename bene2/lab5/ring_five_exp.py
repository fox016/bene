...
class Main(object):
	...
	def parse_options(self):
		...
		parser.add_option("-r", "--runtime", type="int", dest="runtime", default=32)
	...
	def run(self):
		...
		# setup network
		net = Network('networks/five-ring.txt')
		...
		p1 = packet.Packet(source_address=n1.get_address('n5'), destination_address=\
			n5.get_address('n1'), ttl=100, protocol='senddata', body=body)
		Sim.scheduler.add(delay=1, event=p1, handler=n1.send_packet)

		p2 = packet.Packet(source_address=n2.get_address('n3'), destination_address=\
			n4.get_address('n3'), ttl=100, protocol='senddata', body=body)
		Sim.scheduler.add(delay=2, event=p2, handler=n2.send_packet)

		p3 = packet.Packet(source_address=n5.get_address('n4'), destination_address=\
			n3.get_address('n4'), ttl=100, protocol='senddata', body=body)
		Sim.scheduler.add(delay=3, event=p3, handler=n5.send_packet)

		# bring down n1 - n5 link and resend p1
		Sim.scheduler.add(delay=4, event=None, handler=n1.get_link('n5').down)
		Sim.scheduler.add(delay=4, event=None, handler=n5.get_link('n1').down)
		Sim.scheduler.add(delay=24, event=p1, handler=n1.send_packet)

		# bring up n1 - n5 link and resend p1
		Sim.scheduler.add(delay=29, event=None, handler=n1.get_link('n5').up)
		Sim.scheduler.add(delay=29, event=None, handler=n5.get_link('n1').up)
		Sim.scheduler.add(delay=31, event=p1, handler=n1.send_packet)
